...
On first startup, if that file does not exist, the service creates it from the current environment variables and defaults. After that, updates made in the admin console are written back to the same file.
...
When a save changes a restart-required field, the UI reports the exact fields that require restart.
...
9. Change the webhook port and save. Verify the response reports `webhook.port` under `requiresRestart`.
10. Change `workDir` and save. Verify the response reports `workDir` under `requiresRestart`.
