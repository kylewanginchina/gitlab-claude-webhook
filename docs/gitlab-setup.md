...
该模式不会创建分支、提交或 MR。其文本输出中的反引号文件或行引用，例如 `` `src/server/webhookServer.ts:120` ``，会在文件存在时转换为当前 MR 源分支上对应 blob 与行号的 GitLab 源码链接。
...
与自然语言只读 Review 不同，`/code-review` 会生成结构化汇总，并为可定位到 MR 改动行的高置信度 finding 尝试创建 GitLab 行内 discussion。单个行内 discussion 创建失败不会阻止汇总评论发布。
...
每个 `/code-review` 汇总都包含一个隐藏的 head SHA 标记。开启 `REVIEW_SKIP_EXISTING_SHA` 时，服务会检查该标记并跳过已经对同一 MR head SHA 记录过的 Review，避免重复发布。

结构化汇总中的 finding 会链接到该 head SHA 对应的 GitLab blob 和具体代码行；可定位的 finding 还会尝试作为 GitLab 行内 discussion 发布。若 Review pass 或评分阶段部分超时或失败，汇总会明确标注 **partial coverage**、已完成阶段和受影响阶段，不能把这种结果视为完整的无问题 Review。
...
5. 在 MR 上运行 `/code-review`，确认汇总包含 head SHA 标记；若存在高置信度且可定位的 finding，检查其源码链接和行内 discussion。
...
- 若执行过程后 MR head SHA 已变化，服务会跳过发布结果以避免把旧 diff 的结论写到新版本。
