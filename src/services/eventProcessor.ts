      ...
      case 'note':
        if (event.object_attributes) {
          ...
          if (event.issue) {
            ...
            const conversationHistory = await this.getConversationHistory('issue', event.project.id, event.issue.iid, noteId);
            ...
            const conversationHistory = await this.getConversationHistory('merge_request', event.project.id, event.merge_request.iid, noteId);
            if (conversationHistory) {
              context = `${context}\n\n${conversationHistory}`;
            }
          }
